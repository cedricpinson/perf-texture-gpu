...
export const fragmentShaderSourceScrollTexture = `#version 300 es
...
export const fragmentShaderSourceScrollNoTexture = `#version 300 es
precision highp float;
uniform sampler2D u_texture;
uniform uint u_frame;
in vec2 v_texcoord;
out vec4 outColor;
void main() {
    outColor = vec4(1.0, 0.0, v_texcoord.y, 1.0);
}`;


export const funFragmentShaderSource = `#version 300 es
precision highp float;
uniform sampler2D u_texture;
uniform uint u_frame;
in vec2 v_texcoord;
...
{
    float iTime = float(u_frame)/60.0;
    vec2 R = vec2(800.0,600.0);
	...
	O = mix( exp(-33./R.y )* texture( u_texture, (U+vec2(1,sin(t)))/R), // .97
...
void main() {
    ...
    float scrollOffset = float(u_frame)/1024.0;
    scrolledCoord.y = scrolledCoord.y + scrollOffset; // Smooth vertical scrolling with wrap-around using GLSL mod function
    mainImage(outColor, scrolledCoord*vec2(800.0,600.0));
}`;


// https://www.shadertoy.com/view/wtfBDf
export const heavyFragmentShaderSourceWarped = `#version 300 es
precision highp float;
uniform sampler2D u_texture;
uniform uint u_frame;
in vec2 v_texcoord;
...
vec3 getTex(in vec2 p){
    ...
    vec3 tx = texture(u_texture, p/8.).xyz;
    //vec3 tx = textureLod(u_texture, p, 0.).xyz;
...
void main() {
    vec2 scrolledCoord = v_texcoord;
    iTime = float(u_frame)/120.0;
    iResolution = vec3(800.0,600.0,1.0)*0.001;
